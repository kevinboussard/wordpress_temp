...
if ( !function_exists( 'cherry_child_setup' ) ) :
	...
	function cherry_child_setup() {
		...
		set_post_thumbnail_size( 262, 175, true ); // Normal post thumbnails
...
/* Services */
function my_post_type_advantages() {
	register_post_type( 'advantages',
		array(
			'label'             => __('Advantages', CURRENT_THEME),
			'public'            => true,
			'show_ui'           => true,
			'show_in_nav_menus' => false,
			'menu_position'     => 5,
			'rewrite'           => array(
										'slug'       => 'advantages-view',
										'with_front' => FALSE,
									),
			'supports' => array(
								'title',
...
add_action('init', 'my_post_type_advantages');
